...
#              spawns follower objects with dynamic speeds on a timer, and controls
#              their movement and rotation along the curve.
#
@tool
extends Node2D
...
@export var follower_rotation_offset: float = 0.0
# --- NEW: Speed Range ---
@export var follower_speed_min: float = 200.0
@export var follower_speed_max: float = 350.0
# --- NEW: Spawn Timer ---
@export var spawn_on_timer: bool = true
...
@export var handle_factor_a: float = 0.5:
	set(value):
		handle_factor_a = value
		if Engine.is_editor_hint() and is_instance_valid(_curve): _update_curve(); queue_redraw()
@export var handle_rotation_offset_a: float = 0.0:
	set(value):
		handle_rotation_offset_a = value
		if Engine.is_editor_hint() and is_instance_valid(_curve): _update_curve(); queue_redraw()

@export_group("Handle B (End Point)")
@export var handle_mode_b: HandleMode = HandleMode.PERPENDICULAR
@export var handle_factor_b: float = 0.5:
	set(value):
		handle_factor_b = value
		if Engine.is_editor_hint() and is_instance_valid(_curve): _update_curve(); queue_redraw()
@export var handle_rotation_offset_b: float = 0.0:
	set(value):
		handle_rotation_offset_b = value
		if Engine.is_editor_hint() and is_instance_valid(_curve): _update_curve(); queue_redraw()
...
# --- Initialization ---
func _ready():
	# If not in the editor, set up the automatic spawn timer.
	if not Engine.is_editor_hint() and spawn_on_timer:
		var spawn_timer = Timer.new()
		spawn_timer.name = "SpawnTimer"
		spawn_timer.wait_time = spawn_interval
		# Connect the timer's timeout signal to our spawn function.
		spawn_timer.timeout.connect(spawn_follower)
		add_child(spawn_timer)
		spawn_timer.start()

# --- Spawning Logic ---
func _unhandled_input(event: InputEvent) -> void:
	# Manual spawn on keypress is kept for easy testing.
...
func spawn_follower() -> void:
	if not follower_scene:
		...
		return
	var new_node = follower_scene.instantiate(); new_node.scale = Vector2.ONE * follower_scale
	var new_follower = Follower.new(); new_follower.node = new_node; new_follower.progress = 0.0
	# --- NEW: Assign a random speed within the specified range ---
	new_follower.speed = randf_range(follower_speed_min, follower_speed_max)
	add_child(new_node); _followers.append(new_follower)

# --- Core Update Logic ---
func _process(delta: float) -> void:
	if Engine.is_editor_hint(): queue_redraw()
	if not node_a_path.is_empty() and not node_b_path.is_empty(): _update_points(delta)
	if _followers.is_empty() or not is_instance_valid(_curve): return
	...
	for i in range(_followers.size() - 1, -1, -1):
		...
		if follower.progress < 1.0:
			...
			follower.node.queue_free(); _followers.remove_at(i)

# --- Curve Calculation & Drawing (Unchanged) ---
...
func _update_curve() -> void:
	...
	var rot_offset_a_rad = deg_to_rad(handle_rotation_offset_a); var rot_offset_b_rad = deg_to_rad(handle_rotation_offset_b)
	match handle_mode_a:
		HandleMode.PERPENDICULAR: handle_dir_a = (_point_b - _point_a).orthogonal().normalized()
		HandleMode.ROTATION: handle_dir_a = Vector2.RIGHT.rotated(node_a.global_rotation + rot_offset_a_rad)
		HandleMode.VELOCITY:
	...
	match handle_mode_b:
		...
		HandleMode.ROTATION: handle_dir_b = -Vector2.RIGHT.rotated(node_b.global_rotation + rot_offset_b_rad)
		HandleMode.VELOCITY:
	...
	var distance = _point_a.distance_to(_point_b)
	var actual_handle_length_a = distance * handle_factor_a; var actual_handle_length_b = distance * handle_factor_b
	var control_point_1 = _point_a + handle_dir_a * actual_handle_length_a; var control_point_2 = _point_b + handle_dir_b * actual_handle_length_b
